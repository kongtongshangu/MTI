...
class CAVMAEFT(nn.Module):
    ...
    def forward_eval(self, a, v, mode, flag=False, len_keep=None):
        # multi-modal TTA
        if mode == 'multimodal':
            a = a.unsqueeze(1)
            a = a.transpose(2, 3)
            ...
            if flag:
                ...
                for blk in self.blocks_u:
                    au = blk(a) # note here use unified normalization
                au = self.norm(au)
                au = au.mean(dim=1)
                
                for blk in self.blocks_u:
                    vu = blk(v) # note here use unified normalization
                vu = self.norm(vu)
                vu = vu.mean(dim=1)
                ...
                latent_ca = ca.mean(dim=1)
                latent_cv = cv.mean(dim=1)
                x = x.mean(dim=1)
                
                ca = (latent_ca+au) / 2
                cv = (latent_cv+vu) / 2
                
                outputs_a = self.mlp_head(ca)
                outputs_v = self.mlp_head(cv)
                outputs = self.mlp_head(x)

                return outputs, outputs_a, outputs_v
            ...
            if len_keep is not None:
                ...
                index = len_keep.unsqueeze(-1).expand(-1, -1, embed_dim)
            ...
            fea = fea_o.mean(dim=1)

            x = self.mlp_head(fea)

            return x, a, v, attn, fea
    ...
    def forward_cls(self, fea):

        x = self.mlp_head(fea)

        return x
    
    def forward_fusion(self, a, v):
        x = torch.cat((a, v), dim=1)
